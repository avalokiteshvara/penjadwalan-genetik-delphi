...
uses
 Windows, Classes, variants,Contnrs,db,MkSqLite3Api, Dialogs
...
function mkdateToStr(d: tdatetime): string;
begin
// result:=FormatDateTime('yyyy/mm/dd hh:nn:ss.zzz',d,fmt); //D7
 result:=FormatDateTime('yyyy/mm/dd hh:nn:ss.zzz',d); //D6
end;
...
str: TStringList;
begin
  { TODO 1 -otermlich : 02 - Datumskonvertierung }
// result:=sysutils.StrToDateTime(a,fmt); //D7

 result:=sysutils.StrToDateTime(a); //D6
end;
 ...
 case f.datatype of
  ...
  ftDateTime:if cls in [SQLITE_FLOAT,SQLITE_INTEGER] then
              ...
              begin
               ...
               //try
               { TODO 1 -otermlich : 01 - Datumskonvertierung }
                //data:=mkstrToDate(a);
               //except
                data:=a;
               //end;
...
//D6
 { TODO 1 -otermlich : 03 - Datumskonvertierung }
	// DateSeparator:='-';
// DateSeparator:='.';
// TimeSeparator:=':';
	// DecimalSeparator:='.';
