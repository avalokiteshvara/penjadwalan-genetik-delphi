...
uses
  sysutils, classes, uMyDMCT, uMyDMErrors, uMyDMNet
  {$IFDEF HAVE_SSL}, uMyDMSSL{$ENDIF},
  uMyDMNewPassword, SyncObjs;
...
//==============================================================================
  ...
  private
    ...
    fLastRow       : longint;
    fRecList       : TList;    //new for fast search and pos
  ...
  public
    ...
    //Service function
	 function findrows(const Fields: array of Integer; const SearchFields: array of String; ACaseSen : Boolean; APartLen : Integer):int64;
	 procedure SortBy(const Fields: array of Integer; const IsReverseOrder : array of boolean);//mi
  end;
...
//==============================================================================
  ...
  private
    ...
    function read_rows(mysql_fields:PMysql_FieldDef;fields:longint; var hadErrors:boolean; ARecList : TList):PMYSQL_DATA;
...
implementation
uses Math, uMyDMHelpers, mySQLTypes;
...
function TMysqlClient.read_rows(mysql_fields: PMysql_FieldDef; fields: Integer; var hadErrors:boolean; ARecList : TList): PMYSQL_DATA;
...
begin
  ...
  if (field_count = 0) then
    begin
      ...
      if fclient_flag and CLIENT_RESERVED = CLIENT_RESERVED then
        begin
          FWarningCount:=byte(pchar(pos1)[0])+byte(pchar(pos1)[1])shl 8;
          pos1:=pos1+2;
        end;
      //aditional info
  ...
  if fclient_flag and CLIENT_RESERVED = CLIENT_RESERVED then
    fields:=read_rows(nil,7,ae,nil)
  else
    fields:=read_rows(nil,5,ae,nil);
  if fields=nil then
    begin //out of memory?
      result:=-1;
      exit;
...
begin
  ...
  try
    ...
    if fclient_flag and CLIENT_RESERVED = CLIENT_RESERVED then
      fields1:=read_rows(nil,7,ae,nil)
    else
      fields1:=read_rows(nil,5, ae, nil);
    if (fields1 = nil)then
       exit;
    //decode them
...
begin
  ...
  result.fdata:=read_rows(ffields,ffield_count, ae, result.fRecList);
...
function TMysqlClient.GetMultiThreaded: boolean;
begin
  result:=FThreaded;
end;

procedure TMysqlClient.SetMultiThreaded(const Value: boolean);
begin
  if Value then
    begin
...
begin
  ...
  fBOF:=true;
  fLastRow:=0;
  fRecList := TList.Create;  // создаем лист для поиска и позиционировани
end;
...
begin
  ...
  frow:=nil;
  FRecList.Free; //освобождаем лист
  inherited;
end;
    ...
    begin
      if (fdata_cursor<>nil) and (not feof) then
        begin
          ...
          //fdata_cursor := fRecList.Items[fRecList.Count-1];
...
function TMysqlResult.findrows(const Fields: array of Integer; const SearchFields:array of String; ACaseSen : Boolean; APartLen : Integer):int64;
var
  {L,} I, {U,} K   : Integer;
  tt  : PMysql_Rows;
  Cmp : Integer;

function Compare1(const S1: String; const S2 : String; AFldDef : PMysql_FieldDef):Integer;
...
begin
  Result := -1;
    case AFldDef.FieldType of
      FIELD_TYPE_VAR_STRING,
      FIELD_TYPE_STRING: Result := CompWithoutLen(PChar(S1), PChar(S2));
      FIELD_TYPE_TINY,
      FIELD_TYPE_SHORT,
      FIELD_TYPE_LONG,
      FIELD_TYPE_LONGLONG,
      FIELD_TYPE_INT24: if IsZEROFILL(AFldDef)  then
                           Result := CompWithoutLen(PChar(S1), PChar(S2)) else
      ...
      FIELD_TYPE_DATETIME: Result := CompWithoutLen(PChar(S1), PChar(S2));
    end;

end;

function FldVal(CurRow: TMysql_Row; aIndex: Integer): String;
begin
  result:='';
  if (aIndex>-1) and (aIndex< ffieldscount) then //are we in range?
    result:=StrPas(TPPCA(CurRow)[aIndex]); //else we access current row
end;
					 ...
					 begin
						if ACaseSen then
						  begin
							 ...
							 P2 := AnsiUpperCase(FldVal(tt.data, Fields[K]));
						  end else
						  begin
							 P1 := SearchFields[K];
							 P2 := FldVal(tt.data, Fields[K]);
						  end;
						  Cmp := Cmp+Compare1(P1, P2, FieldDef(Fields[K]));
						  if Cmp <> 0 then Break;
					 end;
				  if Cmp = 0 then Break;
				end;
...
procedure TMysqlResult.SortBy(const Fields: array of Integer; const IsReverseOrder : array of boolean);//mi
var
	fd : PMysql_FieldDef;
	p1, p2 : PMysql_Rows;
	SortTypes : array of integer;
	NeedSwap : Boolean;
	FieldsNum, i, tmp : integer;

	procedure SwapData(p1, p2 : PMysql_Rows);
	var
		p_tmp : TMysql_Row;
	begin
		p_tmp := p1^.data;
...
const
	...
	function CmpRecords(p1, p2 : PMysql_Rows; FieldIndex : integer; SortType : integer):integer;
	var
		tmp1, tmp2, sgn : integer;
		str, tmps : string;
	begin
		Result := 0;
		case SortType of
		stString, stDate://mi: dates are represented like 'YYYY-MM-DD HH:MM:SS' so it safe to compare them as strings
			begin
				Result := StrComp(TPPCA(p1.data)[Fields[FieldIndex]],
										TPPCA(p2.data)[Fields[FieldIndex]]);
			end;
		stTime://mi:TIME values may range from '-838:59:59' to '838:59:59', so we need some playing with them
			begin
				str := TPPCA(p1.data)[Fields[FieldIndex]];
				sgn := 1;
				if pos('-', str)<>0 then
					sgn := -1;
				tmps := Copy(str, 0, Pos(':', str) - 1);
				Delete(str, 1, Pos(':', str));
				tmp1 := StrToIntDef(tmps, 0) * 10000;//hours
				tmps := Copy(str, 0, Pos(':', str) - 1);
				Delete(str, 1, Pos(':', str));
				tmp1 := tmp1 + StrToIntDef(tmps, 0) * 100 * sgn;//minutes
				tmp1 := tmp1 + StrToIntDef(str, 0) * sgn;//secs

				str := TPPCA(p2.data)[Fields[FieldIndex]];
				sgn := 1;
				if pos('-', str)<>0 then
					sgn := -1;
				tmps := Copy(str, 0, Pos(':', str) - 1);
				Delete(str, 1, Pos(':', str));
				tmp2 := StrToIntDef(tmps, 0) * 10000;//hours
				tmps := Copy(str, 0, Pos(':', str) - 1);
				Delete(str, 1, Pos(':', str));
				tmp2 := tmp2 + StrToIntDef(tmps, 0) * 100 * sgn;//minutes
				tmp2 := tmp2 + StrToIntDef(str, 0) * sgn;//secs

				Result := tmp1 - tmp2;
			end;
		stNumber:
			begin
				Result := mySign(StrToInt64Def(TPPCA(p1.data)[Fields[FieldIndex]], 0)
									  - StrToInt64Def(TPPCA(p2.data)[Fields[FieldIndex]], 0));
			end;
		stFloat:
			begin
			//mi: D5 has no StrToFloatDef so we need to use try ... except here
				try
				Result := mySign(StrToFloat(TPPCA(p1.data)[Fields[FieldIndex]])
									  - StrToFloat(TPPCA(p2.data)[Fields[FieldIndex]]));
				except
...
begin
	...
	if frowscount < 2 then
		exit;

	FieldsNum := High(Fields) + 1;
	SetLength(SortTypes, FieldsNum);

	for i := 0 to FieldsNum - 1 do
	begin
		SortTypes[i] := stUnavailable;
		fd := FieldDef(Fields[i]);
		case fd^.FieldType of
		FIELD_TYPE_DECIMAL, FIELD_TYPE_TINY, FIELD_TYPE_SHORT,
		FIELD_TYPE_LONG, FIELD_TYPE_INT24,
		FIELD_TYPE_LONGLONG, FIELD_TYPE_YEAR, FIELD_TYPE_NEWDECIMAL:
			begin
				SortTypes[i] := stNumber;
			end;
		FIELD_TYPE_FLOAT, FIELD_TYPE_DOUBLE:
			begin
				SortTypes[i] := stFloat;
			end;
		FIELD_TYPE_NULL, FIELD_TYPE_SET, FIELD_TYPE_TINY_BLOB,
		...
		FIELD_TYPE_NEWDATE, FIELD_TYPE_TIMESTAMP:
			begin
				SortTypes[i] := stDate;
			end;
		FIELD_TYPE_TIME:
		...
		FIELD_TYPE_VAR_STRING, FIELD_TYPE_STRING, FIELD_TYPE_ENUM:
			begin
				SortTypes[i] := stString;
			end;
		end;
	end;

	p1 := fdata.data;//set to first record
	while p1.next <> nil do//mi: simple sort (Later we can implement something more efficient like QuickSort for example)
	begin
		p2 := p1.next;//set to next;
		while p2 <> nil do
		begin
			NeedSwap := false;
			for i := 0 to FieldsNum - 1 do
			begin
				tmp := CmpRecords(p1, p2, i, SortTypes[0]);
				NeedSwap := (tmp > 0) xor IsReverseOrder[i];
				if tmp <> 0 then break;
			end;

			if NeedSwap then SwapData(p1, p2);
			p2 := p2.next;
		end;
		p1 := p1.next;
	end;
