...
{ Simple sign testing
 mi: D5 has no this functions in math unit so we need to implement them here
 I've added 'my' letters just to be on the safe side
 }
...
const
  myNegativeValue = Low(TmyValueSign);
  myZeroValue = 0;
  myPositiveValue = High(TmyValueSign);

function mySign(const AValue: Integer): TmyValueSign; overload;
function mySign(const AValue: Int64): TmyValueSign; overload;
function mySign(const AValue: Double): TmyValueSign; overload;
...
function mySign(const AValue: Integer): TmyValueSign;
begin
  Result := myZeroValue;
  if AValue < 0 then
	 Result := myNegativeValue
  else if AValue > 0 then
	 Result := myPositiveValue;
end;

function mySign(const AValue: Int64): TmyValueSign;
begin
  Result := myZeroValue;
  if AValue < 0 then
	 Result := myNegativeValue
...
function mySign(const AValue: Double): TmyValueSign;
begin
