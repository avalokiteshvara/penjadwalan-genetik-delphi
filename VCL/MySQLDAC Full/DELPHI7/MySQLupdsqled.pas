...
uses Forms, DB, ExtCtrls, StdCtrls, Controls, ComCtrls,
  Classes, SysUtils, Windows, Menus,MySQLDBTables;

type

  TWaitMethod = procedure of object;

  TMySQLUpdateSQLEditForm = class(TForm)
    OkButton: TButton;
  ...
  private
    ...
    FTempTable : TMySQLTable;
    DatabaseOpened: Boolean;
    UpdateSQL: TMySQLUpdateSQL;
    ...
    function TempTable: TMySQLTable;
  end;
...
function EditMySQLUpdateSQL(AUpdateSQL: TMySQLUpdateSQL): Boolean;

implementation

{$R *.DFM}

uses Dialogs, BdeConst, LibHelp, TypInfo;

{ Global Interface functions }

function EditMySQLUpdateSQL(AUpdateSQL: TMySQLUpdateSQL): Boolean;
begin
  with TMySQLUpdateSQLEditForm.Create(Application) do
  try
...
function TMySQLUpdateSQLEditForm.DatabaseOpen: Boolean;
begin
...
function TMySQLUpdateSQLEditForm.Edit: Boolean;
var
  Index: TUpdateKind;
  DataSetName: string;
begin
  Result := False;
  if Assigned(UpdateSQL.DataSet) and (UpdateSQL.DataSet is TMySQLDataSet) then
  begin
    DataSet := TMySQLDataSet(UpdateSQL.DataSet);
...
procedure TMySQLUpdateSQLEditForm.GenWhereClause(const TabAlias, QuoteChar: string;
  KeyFields, SQL: TStrings);
var
  I: Integer;
  BindText: string;
...
procedure TMySQLUpdateSQLEditForm.GenDeleteSQL(const TableName, QuoteChar: string;
  KeyFields, SQL: TStrings);
begin
  SQL.Clear;
...
procedure TMySQLUpdateSQLEditForm.GenInsertSQL(const TableName, QuoteChar: string;
  UpdateFields, SQL: TStrings);
...
procedure TMySQLUpdateSQLEditForm.GenModifySQL(const TableName, QuoteChar: string;
  KeyFields, UpdateFields, SQL: TStrings);
var
  I: integer;
  Comma: string;
...
procedure TMySQLUpdateSQLEditForm.GenerateSQL;
...
procedure TMySQLUpdateSQLEditForm.GetDataSetFieldNames;
begin
...
procedure TMySQLUpdateSQLEditForm.GetTableFieldNames;
begin
...
function TMySQLUpdateSQLEditForm.GetTableRef(const TabName, QuoteChar: string): string;
begin
  if QuoteChar <> '' then
    Result :=  TabName + '.' else
    REsult := '';
...
procedure TMySQLUpdateSQLEditForm.InitGenerateOptions;
var
  UpdTabName: string;
...
procedure TMySQLUpdateSQLEditForm.InitUpdateTableNames;
begin
  UpdateTableName.Items.Clear;
  if Assigned(DataSet) then
  begin
    if DataSet is TMySQLQuery then
      GetSQLTableNames(TMySQLQuery(DataSet).SQL.Text, UpdateTableName.Items)
    else if (DataSet is TMySQLTable) and (TMySQLTable(DataSet).TableName <> '') then
      UpdateTableName.Items.Add(TMySQLTable(DataSet).TableName);
  end;
  if UpdateTableName.Items.Count > 0 then
     UpdateTableName.ItemIndex := 0;
end;

procedure TMySQLUpdateSQLEditForm.SetButtonStates;
begin
...
procedure TMySQLUpdateSQLEditForm.SelectPrimaryKeyFields;
var
...
procedure TMySQLUpdateSQLEditForm.SetDefaultSelections;
var
...
procedure TMySQLUpdateSQLEditForm.ShowWait(WaitMethod: TWaitMethod);
begin
...
function TMySQLUpdateSQLEditForm.TempTable: TMySQLTable;
begin
...
procedure TMySQLUpdateSQLEditForm.FormCreate(Sender: TObject);
begin
  HelpContext := hcDUpdateSQL;
  FTempTable := TMySQLTable.Create(Self);
end;

procedure TMySQLUpdateSQLEditForm.HelpButtonClick(Sender: TObject);
begin
  Application.HelpContext(HelpContext);
end;

procedure TMySQLUpdateSQLEditForm.StatementTypeClick(Sender: TObject);
begin
...
procedure TMySQLUpdateSQLEditForm.OkButtonClick(Sender: TObject);
begin
  if SQLMemo.Modified then
    SQLText[TUpdateKind(StmtIndex)].Assign(SQLMemo.Lines);
end;

procedure TMySQLUpdateSQLEditForm.DefaultButtonClick(Sender: TObject);
begin
  with UpdateTableName do
...
procedure TMySQLUpdateSQLEditForm.GenerateButtonClick(Sender: TObject);
begin
  GenerateSQL;
  FSettingsChanged := False;
end;

procedure TMySQLUpdateSQLEditForm.PrimaryKeyButtonClick(Sender: TObject);
begin
  ShowWait(SelectPrimaryKeyFields);
  SettingsChanged(Sender);
end;

procedure TMySQLUpdateSQLEditForm.PageControlChanging(Sender: TObject;
  var AllowChange: Boolean);
begin
...
procedure TMySQLUpdateSQLEditForm.FormDestroy(Sender: TObject);
begin
...
procedure TMySQLUpdateSQLEditForm.GetTableFieldsButtonClick(Sender: TObject);
begin
...
procedure TMySQLUpdateSQLEditForm.SettingsChanged(Sender: TObject);
begin
...
procedure TMySQLUpdateSQLEditForm.FormCloseQuery(Sender: TObject;
...
procedure TMySQLUpdateSQLEditForm.UpdateTableNameChange(Sender: TObject);
begin
  SettingsChanged(Sender);
end;

procedure TMySQLUpdateSQLEditForm.UpdateTableNameClick(Sender: TObject);
begin
  if not Visible then Exit;
  GetTableFieldsButtonClick(Sender);
end;

procedure TMySQLUpdateSQLEditForm.SelectAllClick(Sender: TObject);
begin
  SelectAll(FieldListPopup.PopupComponent as TListBox);
end;

procedure TMySQLUpdateSQLEditForm.ClearAllClick(Sender: TObject);
var
  I: Integer;
begin
...
procedure TMySQLUpdateSQLEditForm.SQLMemoKeyPress(Sender: TObject;
  var Key: Char);
begin
