...
uses
  sysutils, {$IFDEF DELPHI_5}Windows, {$ENDIF} uMyDMErrors, uMyDMCT {$IFDEF HAVE_SSL},uMyDMSSL{$ENDIF}
  {$IFDEF HAVE_INTSSL},SBClient, SBX509, SBConstants, Classes{$ENDIF};
...
type
  ...
  private
    ...
    {$ENDIF}
    {$IFDEF HAVE_INTSSL}
    procedure FSecureClientSend(Sender: TObject; Buffer: Pointer; Size: Integer);
    procedure FSecureClientReceive(Sender: TObject; Buffer: Pointer; MaxSize: Integer; out Written: Integer);
    procedure FSecureClientData(Sender: TObject; Buffer: Pointer; Size: Integer);
    procedure FSecureClientOpenConnection(Sender: TObject);
    procedure FSecureClientCertificateValidate(Sender: TObject; Certificate: TElX509Certificate; var Validate: Boolean);
    procedure FSecureClientCertificateNeededEx(Sender: TObject; var Certificate: TElX509Certificate);
    {$ENDIF}
  public
...
begin
  ...
  FSecureClient.OnSend:= FSecureClientSend;
  FSecureClient.OnReceive:= FSecureClientReceive;
  FSecureClient.OnData:= FSecureClientData;
  FSecureClient.OnOpenConnection:= FSecureClientOpenConnection;
  FSecureClient.OnCertificateValidate:= FSecureClientCertificateValidate;
  FSecureClient.OnCertificateNeededEx:= FSecureClientCertificateNeededEx;
  FSecureClient.Versions:= [sbSSL3, sbTLS1];
  FCertificate:= nil;
  {$ENDIF}
end;
...
begin
  ...
  if (ftype<>VIO_CLOSED) and (ftype<>VIO_TYPE_NAMEDPIPE) then
    begin
      ...
      FCertificate:= TElX509Certificate.Create(nil);
      try
        _f:= TFileStream.Create(cert, fmopenread);
        try
          FCertificate.LoadFromStreamPEM(_f,'');
        finally
          _f.free;
        end;
        _f:= TFileStream.Create(key, fmopenread);
        try
          FCertificate.LoadKeyFromStreamPEM(_f,'');
        finally
          _f.free;
        end;
      except
        on e:exception do
          begin
            flast_error:= format('Failed to load the certificates (cert:"%s"; key:"%s"). Error: "%s".',[cert, Key, e.Message]);
            flast_errno:=1;
            exit;
          end;
      end;
      ...
      vio_blocking(true);
      setsockopt(FSd, SOL_SOCKET, SO_LINGER, @linger, sizeof(linger));
      FHasError:= false;
      FFirst:= true;
      FSecureClient.Open;
      while not (FSecureClient.Active or FHasError) do
        begin
      ...
      if (ftype =VIO_TYPE_SSL) then //if vio is ssl
        begin
          if Length(FBuffer) > 0 then
            begin
              if sz < Length(FBuffer) then
                begin
                  Move(FBuffer[1], pchar(buf)[0], sz);
                  Delete(FBuffer, 1, sz);
                  Result := sz;
                end
              else
                begin
                  ...
                  Move(FBuffer[1], pchar(buf)[0], Length(FBuffer));
                  SetLength(FBuffer, 0);
                end;
              end
            else
              begin
                ...
                FRecvMaxSize := sz;
                FRecvWritten := 0;
                while not (FDataReceived or FHasError) do
                  begin
                    FSecureClient.DataAvailable;
                    sleep(0);
                  end;
  ...
  if (ftype <> VIO_CLOSED) then //if vio is connected
    begin
      ...
      if (ftype =VIO_TYPE_SSL) then //if vio is ssl
        begin
          result:= size;
          FSecureClient.SendData(@pchar(buf^)[0], size);
          exit;
        end;
      {$ENDIF}
      {$IFDEF HAVE_SSL}
...
{$IFDEF HAVE_INTSSL}
procedure TMysqlVio.FSecureClientCertificateNeededEx(Sender: TObject;
...
procedure TMysqlVio.FSecureClientCertificateValidate(Sender: TObject;
  Certificate: TElX509Certificate; var Validate: Boolean);
var
  _dt: TDateTime;
begin
  _dt:= now;
  { at the moment any certificate is good if not expired}
  validate:= true;
  if Certificate.ValidFrom > _dt then
    validate:= false
  else
    if Certificate.ValidTo < _dt then
      validate:= false;
end;

procedure TMysqlVio.FSecureClientData(Sender: TObject; Buffer: Pointer;
  Size: Integer);
begin
...
procedure TMysqlVio.FSecureClientOpenConnection(Sender: TObject);
begin
  ftype := VIO_TYPE_SSL;
end;

procedure TMysqlVio.FSecureClientReceive(Sender: TObject; Buffer: Pointer;
  MaxSize: Integer; out Written: Integer);
begin
  written:= recv(fsd, Buffer^, MaxSize, 0);
  if written <=0 then
    begin
...
procedure TMysqlVio.FSecureClientSend(Sender: TObject; Buffer: Pointer;
  Size: Integer);
begin
  if send(fsd, buffer^, size,0)<>size then
    begin
      FSecureClient.Close(true);
    end;
end;
